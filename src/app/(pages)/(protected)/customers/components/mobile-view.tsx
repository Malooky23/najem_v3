...
interface MobileViewProps {
  ...
  selectedItem: EnrichedCustomer | null
  setSelectedItem: (item: EnrichedCustomer | null) => void
  activeTab: string
        ...
        globalFilter) && (
          <div className="px-4 py-2 flex flex-wrap items-center gap-2 border-b">
            ...
            {activeFilters.selectedItems.map((id) => {
              const item = data.find((item) => item.customerId === id)
              return (
                <Button
                  key={id}
                  variant="secondary"
                ...
                >
                  {item ? (item.displayName.length > 10 ? item.displayName.substring(0, 10) + "..." : item.displayName) : id}
                  <X className="h-3 w-3" />
                </Button>
              )
            })}
