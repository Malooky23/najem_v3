...
export default function CreateItemForm() {
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    console.log("Touch start - isTouchScrolling:", isTouchScrolling.current);
    console.log("Touched element:", e.target);
  }
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    console.log("Touch move - isTouchScrolling:", isTouchScrolling.current)
  }

  const handleTouchEnd = (e: React.TouchEvent) => {
    // If it was scrolling, prevent focus by stopping propagation
    console.log("Touch end - isTouchScrolling:", isTouchScrolling.current)
    if (isTouchScrolling.current) {
      ...
      if (activeElement && activeElement !== document.body) {
        ...
        // Set focus back to the form container to prevent focus from jumping to drawer button
        if (formRef.current) {
          // Use setTimeout to delay the focus to ensure the blur completes first
          setTimeout(() => {
            // Focus on the form container itself, not any specific interactive element
            formRef.current?.setAttribute('tabindex', '-1');
            formRef.current?.focus();
            formRef.current?.removeAttribute('tabindex');
          }, 0);
        }
      }
    }
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <FormControl>
                      ...
                      >
                        {field.value
                          ? customerOptions.find((customer) => customer.value === field.value)?.label
                          : "Select customer"}
                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                    </FormControl>
