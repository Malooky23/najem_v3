...
export default function ItemsPageWrapper() {
  ...
  useEffect(() => {
    ...
    if (activeFilters.customers.length > 0) {
      table.getColumn("customer")?.setFilterValue(activeFilters.customers)
    }

    // Apply selected items filter
    if (activeFilters.selectedItems.length > 0) {
      table.getColumn("itemId")?.setFilterValue(activeFilters.selectedItems)
    }
  ...
  const handleCustomerFilter = (customer: string) => {
    setActiveFilters((prev) => {
      const customers = prev.customers.includes(customer)
