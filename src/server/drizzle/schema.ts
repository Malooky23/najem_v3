import { pgTable, uuid, text, boolean, timestamp, foreignKey, serial, inet, unique, varchar, integer, check, json, primaryKey, pgView, pgEnum } from "drizzle-orm/pg-core"
...
export const orders = pgTable("orders", {
	...
	orderType: orderType("order_type").default('CUSTOMER_ORDER').notNull(),
...
export const stockMovementsView = pgView("stock_movements_view", {	movementId: uuid("movement_id"),
	itemId: uuid("item_id"),
...
}).as(sql`SELECT sm.movement_id, sm.item_id, sm.location_id, sm.movement_type, sm.quantity, sm.reference_type, sm.reference_id, sm.notes, sm.created_by, sm.created_at, i.item_name, i.customer_id, c.display_name AS customer_display_name FROM stock_movements sm JOIN items i ON sm.item_id = i.item_id JOIN customers c ON i.customer_id = c.customer_id`);